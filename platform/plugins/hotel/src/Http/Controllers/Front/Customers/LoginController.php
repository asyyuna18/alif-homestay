...
class LoginController extends BaseController
{
    ...
    {
        SeoHelper::setTitle(trans('plugins/hotel::customer.login'));

        Theme::breadcrumb()->add(trans('plugins/hotel::customer.login'), route('customer.login'));
        ...
        if ($this->guard()->validate($this->credentials($request))) {
            ...
            if (HotelHelper::isEnableEmailVerification() && empty($customer->confirmed_at)) {
                throw ValidationException::withMessages([
                    'confirmation' => [
                        trans(
                            'plugins/hotel::customer.email_not_confirmed',
                            [
