    ...
    <x-core::modal id="modal-calendar">
        <div class="mb-3">
            <x-core::form.checkbox
                :label="trans('plugins/hotel::room.form.is_available')"
                true-value="1"
                false-value="0"
                v-model="form.active"
            />
        ...
        <div v-show="form.active">
            <x-core::form.text-input
                ...
                :label="trans('plugins/hotel::room.form.value')"
                v-model="form.value"
            />
            ...
            >
                <option :selected="form.value_type === 'fixed'" value="fixed">
                    {{ trans('plugins/hotel::room.form.fixed') }}
                </option>
                <option
                    :selected="form.value_type === 'amount_adjust'"
                    value="amount_adjust"
                >
                    {{ trans('plugins/hotel::room.form.amount_adjust') }}
                </option>
                <option
                    :selected="form.value_type === 'percentage_adjust'"
                    value="percentage_adjust"
                >
                    {{ trans('plugins/hotel::room.form.percentage_adjust') }}
                </option>
            </x-core::form.select>

            <x-core::alert type="info" v-if="form.value_type === 'percentage_adjust'">
                {{ trans('plugins/hotel::room.form.adjust_value_range_info') }}
            </x-core::alert>
        ...
        <x-slot:footer>
            <x-core::button data-bs-dismiss="modal">
                {{ trans('plugins/hotel::room.form.close') }}
            </x-core::button>
            <x-core::button color="primary" @click="saveForm">
                {{ trans('plugins/hotel::room.form.save_changes') }}
            </x-core::button>
        </x-slot:footer>
    </x-core::modal>
</calendar-modal-component>
