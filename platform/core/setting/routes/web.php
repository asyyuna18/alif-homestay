    ...
    AdminHelper::registerRoutes(function (): void {
        Route::prefix('settings')->name('settings.')->group(function (): void {
            ...
            Route::prefix('license')->name('license.')->group(function (): void {
                $verifyLicenseAction = config('core.base.general.skip_license_check')
                    ? static function (BaseHttpResponse $response): BaseHttpResponse {
                        return $response
                            ->setMessage('License check is disabled for this environment.')
                            ->setData([
                                'activated_at' => now()->format('M d Y'),
                                'licensed_to' => setting('licensed_to') ?: config('app.name'),
                            ]);
                    }
                    : 'GeneralSettingController@getVerifyLicense';
